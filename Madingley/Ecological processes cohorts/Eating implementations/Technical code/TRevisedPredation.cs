...
namespace Madingley
{
    ...
    public partial class RevisedPredation : IEatingImplementation
    {
            ...
            madingleyStockDefinitions)
        {
            ...
            PopulateBinnedPreyAbundance(gridCellCohorts, actingCohort, FunctionalGroupIndicesToEat, _PredatorLogOptimalPreyBodySizeRatio);

            //string predatorMetabolism = madingleyCohortDefinitions.GetTraitNames("Endo/Ectotherm", actingCohort[0]);

            // Loop over potential prey functional groups
            foreach (int FunctionalGroup in FunctionalGroupIndicesToEat)
            {
                //if (madingleyCohortDefinitions.GetTraitNames("Endo/Ectotherm", FunctionalGroup) == predatorMetabolism)
                {
                    ...
                    if (_DietIsAllSpecial)
                    {
                        ...
                        for (int i = 0; i < NumberCohortsPerFunctionalGroupNoNewCohorts[FunctionalGroup]; i++)
                        {
                            ...
                            PreyMassBinNumber = GetBinNumber(_BodyMassPrey, _BodyMassPredator, _PredatorLogOptimalPreyBodySizeRatio);
        ...
        private int GetBinNumber(double preyMass, double predatorMass, double predatorOptimalPreyBodySizeRatio)
        {
        ...
        private double GetBinNumberFractional(double preyMass, double predatorMass, double predatorOptimalPreyBodySizeRatio)
        {
            return (Math.Log(preyMass / predatorMass) - predatorOptimalPreyBodySizeRatio) / 
            ...
            foreach (int FunctionalGroup in _FunctionalGroupIndicesToEat)
            {
                ...
                for (int i = 0; i < NumberCohortsPerFunctionalGroupNoNewCohorts[FunctionalGroup]; i++)
                {
                    ...
                    if (trackProcesses.TrackProcesses)
                    {
                        ...
                        {
                            if (RandomNumberGenerator.GetUniform() > 0.9)
