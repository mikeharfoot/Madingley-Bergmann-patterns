    ...
    public partial class ReproductionBasic : IReproductionImplementation
    {
        ...
        double ReproductiveMassIncludingChangeThisTimeStep;
            ...
            if ((CurrentMassRatio > _MassRatioThreshold) && ((cellEnvironment["Breeding Season"][currentMonth] == 1.0) || ((cellEnvironment["Realm"][0] == 2.0))))
            {
                ...
                if (tracker.TrackProcesses)
                    tracker.RecordNewCohort((uint)cellEnvironment["LatIndex"][0],  (uint)cellEnvironment["LonIndex"][0],
                        currentTimestep, _OffspringCohortAbundance, OffspringJuvenileAndAdultBodyMasses[0], gridCellCohorts[actingCohort].AdultMass, gridCellCohorts[actingCohort].FunctionalGroupIndex,
                        gridCellCohorts[actingCohort].CohortID, (uint)partial.NextCohortIDThreadLocked, gridCellCohorts[actingCohort].CohortAbundance);
            ...
            else
            {
                ...
                if (tracker.TrackProcesses)
                    tracker.RecordNewCohort((uint)cellEnvironment["LatIndex"][0], (uint)cellEnvironment["LonIndex"][0],
                        currentTimestep, 0, OffspringJuvenileAndAdultBodyMasses[0], gridCellCohorts[actingCohort].AdultMass, gridCellCohorts[actingCohort].FunctionalGroupIndex,
                        gridCellCohorts[actingCohort].CohortID, (uint)partial.NextCohortIDThreadLocked, gridCellCohorts[actingCohort].CohortAbundance);
            }
