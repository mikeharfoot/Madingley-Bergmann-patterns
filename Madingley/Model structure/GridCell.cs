    ...
    public class GridCell
    {
            ...
            bool specificLocations)
        {
            ...
            if (_CellEnvironment.ContainsKey("LandSeaMask"))
            {
                ...
                {
                    if (! ContainsData(_CellEnvironment["NPP_1"], _CellEnvironment["Missing Value"][0]))
                    {
                        ...
                        if (_CellEnvironment.ContainsKey("Temperature"))
                        {
                            ...
                            if (!_CellEnvironment.ContainsKey("TemperatureNPP"))
                            {
                                var tempVector2 = _CellEnvironment["Temperature"].ToArray();
                                _CellEnvironment["TemperatureNPP"] = tempVector2;
                            }
                        }
                        else
                        {
            ...
            meantemp = CalculateAnnualMean(tempVector, missingValue);

            var subset = from i in tempVector where i != missingValue select i;
            double SumOfSquaresDifferences = subset.Select(val => (val - meantemp[0]) * (val - meantemp[0])).Sum();
            sdtemp[0] = Math.Sqrt(SumOfSquaresDifferences / subset.Count());

            if (double.IsNaN(sdtemp[0]) || double.IsInfinity(sdtemp[0]))
            {
                sdtemp[0] = missingValue;
            }
            ...
            //Remove spatial variation in mean conditions - make all cells equivalent to equatorial mean conditions
            ...
            //Adjust the mean to be equal to the equatorial average conditions
            // Marine: NPP, Temp and DTR were done in the marine terrestrial split above
            // Terrestrial: Temp and DTR were done above 
            //tempVector = CalculateSeasonality(_CellEnvironment["Precipitation"], _CellEnvironment["Missing Value"][0]);
            //double scaler = 150.0;
            //for (int i = 0; i < _CellEnvironment["Precipitation"].Length; i++)
            //{
            //    //if (_CellEnvironment["Precipitation"][i] != _CellEnvironment["Missing Value"][0])
            //        //_CellEnvironment["Precipitation"][i] = tempVector[i] * scaler;
            //        //_CellEnvironment["Precipitation"][i] = scaler;
            //}
            //tempVector = CalculateSeasonality(_CellEnvironment["FrostDays"], _CellEnvironment["Missing Value"][0]);
            //scaler = 0.2;
            //for (int i = 0; i < _CellEnvironment["FrostDays"].Length; i++)
            //{
            //    //if (_CellEnvironment["FrostDays"][i] != _CellEnvironment["Missing Value"][0])
            //        //_CellEnvironment["FrostDays"][i] = tempVector[i] * scaler;
            //        //_CellEnvironment["FrostDays"][i] = scaler;
            //}

            //// Remove seasonality from the model
            //tempVector = _CellEnvironment["Temperature"];
            //Average = 0.0;
            //int cnt = 0;
            //for (int i = 0; i < 12; i++)
            //{
            //    if (tempVector[i] != _CellEnvironment["Missing Value"][0])
            //    {
            //        Average += tempVector[i];
            //        cnt++;
            //    }
            //}
            //if (cnt > 0)
            ...
            //else
            //{
            //    Average = _CellEnvironment["Missing Value"][0];
            //}

            //for (int i = 0; i < tempVector.Length; i++)
            //{
            //    _CellEnvironment["Temperature"][i] = Average;
            //}
            //tempVector = _CellEnvironment["TemperatureNPP"];
            //Average = 0.0;
            //cnt = 0;
            //for (int i = 0; i < 12; i++)
            //{
            ...
            //for (int i = 0; i < tempVector.Length; i++)
            //{
            //    _CellEnvironment["TemperatureNPP"][i] = Average;
            //}
            //tempVector = _CellEnvironment["DiurnalTemperatureRange"];
            //Average = 0.0;
            //cnt = 0;
            //for (int i = 0; i < 12; i++)
            //{
            ...
            //for (int i = 0; i < tempVector.Length; i++)
            //{
            //    _CellEnvironment["DiurnalTemperatureRange"][i] = Average;
            //}
            //tempVector = _CellEnvironment["NPP"];
            //Average = 0.0;
            //cnt = 0;
            //for (int i = 0; i < 12; i++)
            //{
            ...
            //for (int i = 0; i < tempVector.Length; i++)
            //{
            //    _CellEnvironment["NPP"][i] = Average;
            //}
            //tempVector = _CellEnvironment["Precipitation"];
            //Average = 0.0;
            //cnt = 0;
            //for (int i = 0; i < 12; i++)
            //{
            ...
            //for (int i = 0; i < tempVector.Length; i++)
            //{
            //    _CellEnvironment["Precipitation"][i] = Average;
            //}
            //tempVector = _CellEnvironment["FrostDays"];
            //Average = 0.0;
            //cnt = 0;
            //for (int i = 0; i < 12; i++)
            //{
            ...
            //for (int i = 0; i < tempVector.Length; i++)
            //{
            //    _CellEnvironment["FrostDays"][i] = Average;
            //}
            ...
            frostMonthly[0] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][0] / 31.0), 1.0);
            frostMonthly[1] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][1] / 28.0), 1.0);
            frostMonthly[2] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][2] / 31.0), 1.0);
            frostMonthly[3] = Math.Min(Math.Max(0, _CellEnvironment["FrostDays"][3] / 30.0), 1.0);
            frostMonthly[4] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][4] / 31.0), 1.0);
            frostMonthly[5] = Math.Min(Math.Max(0, _CellEnvironment["FrostDays"][5] / 30.0), 1.0);
            frostMonthly[6] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][6] / 31.0), 1.0);
            frostMonthly[7] = Math.Min(Math.Max(0, _CellEnvironment["FrostDays"][7] / 31.0), 1.0);
            frostMonthly[8] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][8] / 30.0), 1.0);
            frostMonthly[9] = Math.Min(Math.Max(0, _CellEnvironment["FrostDays"][9] / 31.0), 1.0);
            frostMonthly[10] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][10] / 30.0), 1.0);
            frostMonthly[11] = Math.Min(Math.Max(0,_CellEnvironment["FrostDays"][11] / 31.0), 1.0);
            ...
            double[] BreedingSeason = CalculateBreedingSeason();
        ...
        public double[] CalculateBreedingSeason()
        {
        ...
        private double CalculateEnvironmentalAverage(double[] tempVector, double missingValue)
        {
        ...
        public double[] ConvertMissingValuesToZero(double[] data, double missingValue)
        {
            ...
            {
                TempArray[ii] = (TempArray[ii].CompareTo(missingValue) != 0 && !(double.IsNaN(data[ii]))) ? TempArray[ii] : 0.0;
            }
        ...
        public Boolean ContainsData(double[] data, double missingValue)
        {
            ...
            {
                if (data[ii].CompareTo(missingValue) != 0 && !(double.IsNaN(data[ii]))) ContainsData = true;
            }
            return ContainsData;
        }
        ...
        public double[] CalculateSeasonality(double[] vals, double missingValue)
        {
            double[] TempVals = new double[vals.Length];

            
            //First normalise the values by subtracting the minumum value (this should make the series minimum equal to 0)
            double MinVal = 0;
            double TotVal = 0;
            int NumVals = 0;
            for (int i = 0; i < vals.Length; i++)
            {
                //Find the minimum value that is not a missing value
                if (vals[i].CompareTo(missingValue) != 0)
                {
                    if (vals[i].CompareTo(MinVal) < 0) MinVal = vals[i];
                }
            }
            for (int i = 0; i < vals.Length; i++)
            {
        ...
        public double[] CalculateAnnualMean(double[] data, double missingValue)
        {
            double mean = 0.0;
            double[] MeanVector = new double[12];

            var subset = from i in data where i != missingValue select i;
            mean = subset.Sum() / subset.Count();
        ...
        private void SeedGridCellCohorts(ref FunctionalGroupDefinitions functionalGroups, ref SortedList<string, double[]>
            cellEnvironment, SortedList<string, double> globalDiagnostics, ref Int64 nextCohortID, Boolean tracking, double totalCellTerrestrialCohorts, 
            double totalCellMarineCohorts, Boolean DrawRandomly, Boolean ZeroAbundance)
        {
                    ...
                    if (FunctionalGroupsToUse.Contains(FunctionalGroup))
                    {
                        ...
                        for (int jj = 0; jj < NumberOfCohortsInThisFunctionalGroup; jj++)
                        {
                            ...
                            */
                            ...
                            OptimalPreyBodySizeRatio, (ushort)0, ProportionTimeActive[FunctionalGroup], ref nextCohortID,TrophicIndex, tracking);
