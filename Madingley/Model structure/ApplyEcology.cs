...
namespace Madingley
{
    ...
    public class ApplyEcology
    {
            ...
            uint currentTimestep, ProcessTracker tracker, SortedList<string,double[]> cellEnvironment)
        {
            ...
            // Specifically only output the growth information for the last 12 timesteps
            //if(currentTimestep > 1187)
            {
                ...
                tracker.TrackTimestepGrowth((uint)cellEnvironment["LatIndex"][0], (uint)cellEnvironment["LonIndex"][0], currentTimestep,
                    gridCellCohorts[actingCohort].IndividualBodyMass, gridCellCohorts[actingCohort].FunctionalGroupIndex, growth,
                    deltaBiomass["metabolism"], deltaBiomass["predation"], deltaBiomass["herbivory"], gridCellCohorts[actingCohort].AdultMass);
