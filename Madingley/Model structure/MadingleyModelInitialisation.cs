...
namespace Madingley
{
    ...
    public class MadingleyModelInitialisation
    {
        ...
        /// </summary>
        private int _CellRarefaction;
        /// <summary>
        /// Get and set the rarefaction of active cells in the model grid
        /// </summary>
        public int CellRarefaction
        {
            get { return _CellRarefaction; }
            set { _CellRarefaction = value; }
        }
        ...
        public MadingleyModelInitialisation(string initialisationFile, string outputPath)
        {
            // Write to console
        ...
        public void ReadAndCopyInitialisationFiles(string initialisationFile, string outputPath)
        {
            // Construct file name
            ...
            for (int row = 0; row < VarParameters.Length; row++)
            {
                ...
                switch (VarParameters.GetValue(row).ToString().ToLower())
                {
                    ...
                    case "recovery duration (years)":
                        ...
                        break;
                    case "instantaneous impact (year)":
                        _InstantaneousTimeStep = (uint)Utilities.ConvertTimeUnits("year", _GlobalModelTimeStepUnit) * Convert.ToUInt32(VarValues.GetValue(row));
                        break;
                    case "number instantaneous impact steps (year)":
                        _NumInstantaneousTimeStep = Convert.ToUInt32(Utilities.ConvertTimeUnits("year", _GlobalModelTimeStepUnit) * Convert.ToSingle(VarValues.GetValue(row)));
                        break;
                    ...
                    case "read state":
                        ...
                        {
                            _InputState = true;
                            this.ReadModelState(VarValues.GetValue(row).ToString(), outputPath);
                        }
                        break;
                    ...
                    case "impact cell index":
                        if (VarValues.GetValue(row).ToString() != "")
                        {
                            ...
                            {
                                _ImpactCellIndices.Add(Convert.ToUInt32(temp[ii]));
                            }
                        }
                        break;
                    case "output detail":
        ...
        private void ReadModelState(string modelStateFileSpecification, string outputPath)
        {
            List<string> ModelStatePath = new List<string>();
            List<string> ModelStateFilename = new List<string>();

            ModelStates = new List<InputModelState>();

            string FileString = "msds:csv?file=input/Model setup/" + modelStateFileSpecification + "&openMode=readOnly";

            //Copy the file containing the list of environmental layers to the output directory
            System.IO.File.Copy("input/Model setup/" + modelStateFileSpecification, outputPath + modelStateFileSpecification, true);
            ...
            foreach (Variable v in InternalData.Variables)
            {
                ...
                {
                    case "path":
                        for (int ii = 0; ii < TempValues.Length; ii++) ModelStatePath.Add(TempValues.GetValue(ii).ToString());
                        break;
                    case "filename":
                        for (int ii = 0; ii < TempValues.Length; ii++) ModelStateFilename.Add(TempValues.GetValue(ii).ToString());
                        break;
                }
            }
