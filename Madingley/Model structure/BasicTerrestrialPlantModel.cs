...
{
    /// <summary>
    /// Basic version of Matt Smith's terrestrial carbon model
    /// </summary>
    public class BasicTerrestrialPlantModel
    {
        ...
        /// <summary>
        /// The minimum fraction of productivity that can be allocated to structural tissue
        /// </summary>
        private double MinFracStruct;
        ...
        /// </summary>
        public BasicTerrestrialPlantModel()
        {
        ...
        public void InitialisePlantModelParameters()
        {
            ...
            MinFracStruct = 0.01; // This prevents the prediction becoming zero (makes likelihood calculation difficult)
        ...
        public void WriteOutParameterValues(StreamWriter sw)
        {
            ...
            sw.WriteLine("Terrestrial Plant Model\tMinFracStruct\t" + Convert.ToString(MinFracStruct));
        ...
        public double CalculateEquilibriumLeafMass(SortedList<string, double[]> cellEnvironment, bool deciduous)
        {
            ...
            double FracEvergreen = this.CalculateFracEvergreen(MeanTemp, TotalPrecip, cellEnvironment["Fraction Year Frost"][0]);
            ...
            double LeafMortEvergreen = this.CalculateEvergreenLeafMortality(MeanTemp);

            // Calculate deciduous leaf mortality
            double LeafMortDeciduous = this.CalculateDeciduousLeafMortality(MeanTemp);


            // Calculate leaf mortality rate
            double LeafMortRate = Math.Exp(FracEvergreen * Math.Log(LeafMortEvergreen) + (1 - FracEvergreen) * Math.Log(LeafMortDeciduous));

            
            // Calculate the fractional mortality of leaves
            double LeafMortFrac = this.CalculateLeafMortalityFraction(LeafMortRate, FRootMort);
            ...
            // Calculate the actual leaf matter for this stock depending on whether leaves are deciduous or evergreen
            if (deciduous)
            {
                LeafWetMatter *= (1 - FracEvergreen);
            }
            else
            {
                LeafWetMatter *= FracEvergreen;
            }
        ...
        public void UpdateLeafStock(SortedList<string, double[]> cellEnvironment, GridCellStockHandler gridCellStocks, int[] actingStock, 
            uint currentTimeStep, bool deciduous, string GlobalModelTimeStepUnit, uint currentMonth)
        {
            // ESTIMATE ANNUAL LEAF CARBON FIXATION ASSUMING ENVIRONMENT THROUGHOUT THE YEAR IS THE SAME AS IN THIS MONTH

            // Calculate NPP using the Miami model
            double NPP = this.CalculateMiamiNPP(cellEnvironment["Temperature"][currentMonth], cellEnvironment["Precipitation"][currentMonth] * 12);

            // Calculate fractional allocation to structural tissue
            double FracStruct = this.CalculateFracStruct(NPP);

            // Calculate the fractional allocation of NPP to evergreen plant matter
            double FracEvergreen = this.CalculateFracEvergreen(cellEnvironment["Temperature"][currentMonth], cellEnvironment["Precipitation"][currentMonth] * 12, cellEnvironment["Fraction Year Frost"][0]);

            // Calculate evergreen leaf mortality
            double LeafMortEvergreen = this.CalculateEvergreenLeafMortality(cellEnvironment["Temperature"][currentMonth]);

            // Calculate deciduous leaf mortality
            double LeafMortDeciduous = this.CalculateDeciduousLeafMortality(cellEnvironment["Temperature"][currentMonth]);

            // Calculate leaf mortality rate
            double LeafMortRate = Math.Exp(FracEvergreen * Math.Log(LeafMortEvergreen) + (1 - FracEvergreen) * Math.Log(LeafMortDeciduous));

            // Calculate fine root mortality rate
            double FRootMort = this.CalculateFineRootMortalityRate(cellEnvironment["Temperature"][currentMonth]);
            ...
            // ESTIMATE THE MONTHLY CHANGE IN LEAF WET MATTER BASED ON THE ANNUAL ESTIMATE OF LEAF CARBON FIXATION

            // Get the scalar to convert from the time step used by the plant model (year) to the global model time step unit
            double DeltaT = Utilities.ConvertTimeUnits(GlobalModelTimeStepUnit, "year");

            // Calculate current leaf carbon in gC/m2
            // Get leaf mass wet mass in g in the grid cell

            double LeafMass = 0.0;

            for (int i = 0; i < gridCellStocks.Count; i++)
            {
                foreach (Stock item in gridCellStocks[i])
                {
                    LeafMass += item.TotalBiomass;
                }
            }
            
            // Convert to kg carbon per m2
            double LeafCarbon = this.ConvertToKgCarbonPerM2(LeafMass, cellEnvironment["Cell Area"][0]);

            // Calculate leaf C mortality (g C that would be lost to mortality in a year assuming constant environmental conditions)
            double LeafCMortality = LeafMortRate * LeafCarbon;

            // Calculate plant carbon increment (converting to a monthly estimated value)
            double CarbonIncrement = (LeafCFixation - LeafCMortality) * DeltaT;

            // Convert from carbon to leaf wet matter
            double WetMatterIncrememnt = this.ConvertToLeafWetMass(CarbonIncrement, cellEnvironment["Cell Area"][0]);

            // Calculate the increment in leaf matter for this stock depending on whether the leaves are deciduous or evergreen
            if (deciduous)
            {
                WetMatterIncrememnt *= (1 - FracEvergreen);
            }
            else
            {
                WetMatterIncrememnt *= FracEvergreen;
            }
            
            // Add the leaf wet matter to the acting stock
            gridCellStocks[actingStock].TotalBiomass += WetMatterIncrememnt;

        }

        /// <summary>
        /// Calculate the standing leaf mass for a stock given current environmental conditions, based on the calculation of leaf mass at equilibrium 
        /// </summary>
        /// <param name="cellEnvironment">The environment in the current grid cell</param>
        /// <param name="gridCellStocks">The stocks in the current grid cell</param>
        /// <param name="actingStock">The position of the acting stock in the array of grid cell stocks</param>
        ...
        /// <param name="deciduous">Whether the acting stock consists of deciduous leaves</param>
        /// <param name="currentMonth">The current model month</param>
        public void DecoupledLeafStockUpdate(SortedList<string, double[]> cellEnvironment, GridCellStockHandler 
            gridCellStocks, int[] actingStock, uint currentTimeStep, bool deciduous, uint currentMonth)
        {
            // Calculate the maximum equilibrium leaf mass
            double MaximumLeafMass = this.CalculateEquilibriumLeafMass(cellEnvironment, deciduous);

            // Calculate the fractional allocation of NPP to evergreen plant matter
            double FracEvergreen = this.CalculateFracEvergreen(cellEnvironment["Temperature"][currentMonth], cellEnvironment["Precipitation"][currentMonth] * 12, cellEnvironment["Fraction Year Frost"][0]);

	        // Get the proportional contribution of this month to annual productivity
            double MonthProductivityProportion = cellEnvironment["Seasonality"][currentMonth];

            if (deciduous)
            {

                // Estimate the standing leaf stock this month as a proportion of the maximum, assuming that this scales from 25% of the maximum in 
                // the least productive months to 100% of the maximum in the most productive months
                double MonthLeafMassProportion = MonthProductivityProportion * 0.75 + 0.25;

                // Update the autotroph stock total biomass with this value
                gridCellStocks[actingStock].TotalBiomass = MaximumLeafMass * MonthLeafMassProportion * (1 - FracEvergreen);

            }
            else
            {
                // Update the autotroph stock total biomass with the maximum value, assuming no seasonal variation for evergreen leaves
                gridCellStocks[actingStock].TotalBiomass = MaximumLeafMass * FracEvergreen;
            }
        ...
        public double CalculateFracEvergreen(double temperature, double precipitation, double NDF)
        {
        ...
        public double CalculateEvergreenLeafMortality(double temperature)
        {
        ...
        public double CalculateDeciduousLeafMortality(double temperature)
        {
        ...
        /// <summary>
        /// Calculate the fractional mortality of all leaves
        /// </summary>
        ...
        public double CalculateLeafMortalityFraction(double LeafMortRate, double FRootMort)
        {
        ...
        /// <param name="cellArea">The area of the grid cell in km2</param>
        ...
        public double ConvertToKgCarbonPerM2(double leafWetMatter, double cellArea)
        {
            ...
            double LeafDryMatter = leafWetMatter * 0.213;

            // Convert from dry matter to g C per grid cell
            double gCarbonPerCell = LeafDryMatter * 0.476;
