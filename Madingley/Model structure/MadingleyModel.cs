...
{   
    ...
    public class MadingleyModel
    {
        ...
        {         
            ...
            HarvestingSimulator = new Harvesting();
        }
        ...
        {            
            ...
            Boolean varExists;
             ...
             for (UInt32 hh = 0; hh < NumTimeSteps; hh += 1)
             {
                 ...
                 //if (CurrentTimeStep == (3 * 12))
                 //{
                 //    for (int i = 0; i < _CellList.Count; i++)
                 //    {
                 //        ProcessTrackers[i].TrackProcesses = false;
                 //    }

                 //    TrackCrossCellProcesses.TrackCrossCellProcesses = false;

                 //    TrackGlobalProcesses.TrackProcesses = false;

                 //}
                 //else if (CurrentTimeStep >= (NumTimeSteps - (3 * 12)))
                 //{
                 //    for (int i = 0; i < _CellList.Count; i++)
                 //    {
                 //        ProcessTrackers[i].TrackProcesses = true;
                 //    }

                 //    TrackCrossCellProcesses.TrackCrossCellProcesses = true;

                 //    TrackGlobalProcesses.TrackProcesses = true;
                 ...
                 if (TrackGlobalProcesses.TrackProcesses)
                 {
                     for (uint ii = 0; ii < StockFunctionalGroupDefinitions.GetNumberOfFunctionalGroups(); ii++)
                     {
                         ...
                         //TrackGlobalProcesses.StoreLeafMortalityGrid(hh);
                 ...
                 for (int i = 0; i < _CellList.Count; i++)
                 {
                     if (ProcessTrackers[i].TrackProcesses)
                         ProcessTrackers[i].FlushStreams();
                 }

                 if(TrackCrossCellProcesses.TrackCrossCellProcesses) TrackCrossCellProcesses.FlushStreams();
            ...
            MadingleyModelInitialisation initialisation)
        {
            ...
            ClimateChangeSimulator.ApplyTemperatureScenario(
                ...
                (initialisation.ImpactCellIndices.Contains((uint)cellIndex)));
            ...
            string outputFilesSuffix)
        {
            ...
            if (!SpecificLocations)
            {
                if(!EnviroStack.ContainsKey("Temperature")) EnviroStack.Add("Temperature", new EnviroData("temperature", "month", BottomLatitude, LeftmostLongitude, TopLatitude, RightmostLongitude, CellSize, EnvironmentalDataSource.ANY));
                if (!EnviroStack.ContainsKey("LandDTR")) EnviroStack.Add("LandDTR", new EnviroData("land_dtr", "month", BottomLatitude, LeftmostLongitude, TopLatitude, RightmostLongitude, CellSize, EnvironmentalDataSource.ANY));
                //if (!EnviroStack.ContainsKey("OceanTemp")) EnviroStack.Add("OceanTemp", new EnviroData("temperature_ocean", "month", BottomLatitude, LeftmostLongitude, TopLatitude, RightmostLongitude, CellSize, EnvironmentalDataSource.ANY));
                if (!EnviroStack.ContainsKey("Precipitation")) EnviroStack.Add("Precipitation", new EnviroData("precipitation", "month", BottomLatitude, LeftmostLongitude, TopLatitude, RightmostLongitude, CellSize, EnvironmentalDataSource.ANY));
                if (!EnviroStack.ContainsKey("FrostDays")) EnviroStack.Add("FrostDays", new EnviroData("frost", "month", BottomLatitude, LeftmostLongitude, TopLatitude, RightmostLongitude, CellSize, EnvironmentalDataSource.ANY));
            }
            ...
            ScenarioParameterInitialisation scenarioParameters, int scenarioIndex, int simulation)
        {
            ...
            else
            {
                ...
                for (uint ii = 0; ii < NumLatCells; ii += (uint)CellRarefaction)
                {
                    for (uint jj = 0; jj < NumLonCells; jj += (uint)CellRarefaction)
                    {
            ...
            else
            {
                EcosystemModelGrid.SeedGridCellStocksAndCohorts(_CellList, CohortFunctionalGroupDefinitions, StockFunctionalGroupDefinitions,
                    GlobalDiagnosticVariables, ref NextCohortID, InitialisationFileStrings["OutputDetail"] == "high", DrawRandomly, initialisation.DispersalOnly, InitialisationFileStrings["DispersalOnlyType"]);
            }
        ...
        public void InitialOutputs(string outputFilesSuffix, MadingleyModelInitialisation initialisation, uint month)
        {
            ...
            else
            {
                ...
                GridOutputs.InitialOutputs(EcosystemModelGrid, CohortFunctionalGroupDefinitions, StockFunctionalGroupDefinitions, _CellList, initialisation, month);
            }
            ...
            foreach (int FunctionalGroup in AutotrophStockFunctionalGroups)
            {
                for (int ll = 0; ll < workingGridCellStocks[FunctionalGroup].Count; ll++)
                {
                    ...
                    MadingleyEcologyStock.RunWithinCellEcology(workingGridCellStocks, ActingStock, EcosystemModelGrid.GetCellEnvironment(
                        ...
                        InitialisationFileStrings["OutputDetail"],SpecificLocations,(initialisation.ImpactCellIndices.Contains((uint)cellIndex)));
            ...
            foreach (uint[] cell in _CellList)
            {
                if (ProcessTrackers[i].TrackProcesses)
                {
                    ...
                    for (int FunctionalGroup = 0; FunctionalGroup < TempCohorts.Count; FunctionalGroup++)
                    {
                        ...
                        {
                            ProcessTrackers[i].RecordNewCohort(cell[0], cell[1], 0, item.CohortAbundance, item.IndividualBodyMass, item.AdultMass, item.FunctionalGroupIndex,
                                new List<uint> { uint.MaxValue }, item.CohortID[0],item.CohortAbundance);
                        }
                    }
                }
                i += 1;
