...
namespace Madingley
{
    ...
    public class ReproductionTracker
    {
            ...
            string outputPath, int cellIndex)
        {
            ...
            SyncNewCohortWriter.WriteLine("Latitude\tLongitude\ttime_step\tabundance\toffspringBodymass\tfunctional group\tadult mass\tparent cohort IDs\toffspring cohort ID\tparent abundance");
            ...
            SyncMaturityWriter.WriteLine("Latitude\tLongitude\ttime_step\tbirth_step\tjuvenile Mass\tadult mass\tfunctional group\tparent abundance");
        ...
        public void RecordNewCohort(uint latIndex, uint lonIndex, uint timestep, double offspringCohortAbundance, double offspringBodymass, double parentCohortAdultMass, 
            int functionalGroup, List<uint> parentCohortIDs,uint offspringCohortID, double parentCohortAbundance)
        {
            ...
            string newline = Convert.ToString(latIndex) + '\t' + Convert.ToString(lonIndex) + '\t' +
                Convert.ToString(timestep) + '\t' + Convert.ToString(offspringCohortAbundance) + '\t' + Convert.ToString(offspringBodymass) + '\t' +
                Convert.ToString(functionalGroup) + '\t' + Convert.ToString(parentCohortAdultMass) + '\t'  + AllCohortIDs +
                '\t' + Convert.ToString(offspringCohortID) + '\t' + Convert.ToString(parentCohortAbundance);
        ...
        {
            SyncMaturityWriter.Flush();
            MaturityWriter.Flush();
            SyncNewCohortWriter.Flush();
            NewCohortWriter.Flush();
        }
