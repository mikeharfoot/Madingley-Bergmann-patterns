...
namespace Madingley
{
    ...
    public class GrowthTracker
    {
            ...
            string outputFilesSuffix, string outputPath, int cellIndex)
        {
            ...
            SyncGrowthWriter.WriteLine("Latitude\tLongitude\ttime_step\tCurrent_body_mass_g\tfunctional_group\tgrowth_g\tmetabolism_g\tpredation_g\therbivory_g\tAdult_mass");
        ...
        public void RecordGrowth(uint latIndex, uint lonIndex, uint timeStep, double currentBodyMass, int functionalGroup, 
            double netGrowth, double metabolism, double predation, double herbivory, double adultMass)
        {
            SyncGrowthWriter.WriteLine(Convert.ToString(latIndex) + '\t' + Convert.ToString(lonIndex) + '\t' + Convert.ToString(timeStep) +
                '\t' + Convert.ToString(currentBodyMass) + '\t' + Convert.ToString(functionalGroup) + '\t' + Convert.ToString(netGrowth) +
                '\t' + Convert.ToString(metabolism) + '\t' + Convert.ToString(predation) + '\t' + Convert.ToString(herbivory) + '\t' + Convert.ToString(adultMass));
